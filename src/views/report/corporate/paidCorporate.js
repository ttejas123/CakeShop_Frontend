...
const columns = [
  ...
  const handleFilterByDropDown = (value) => {
    ...
    setSearchValue(search)
      if (search.length) {
          updatedData = paidCorporateData.filter(item => {
            const startsWith =
  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = paidCorporateData.filter(item => {
        const startsWith =
          item.corporate.toLowerCase().startsWith(value.toLowerCase())
        const includes =
          item.corporate.toLowerCase().includes(value.toLowerCase())
  ...
  const CustomPagination = () => (
    <ReactPaginate
      ...
      pageCount={searchValue.length ? filteredData.length / rowsPerPage : paidCorporateData.length / rowsPerPage || 1}
      breakLabel='...'
    ...
    <Fragment>
      ...
      <Card>
        ...
        <DataTable
          ...
          data={searchValue.length ? filteredData : paidCorporateData}
